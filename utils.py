...
def cast_df(df, col, d_type=str):
    return df.loc[:, col].astype(d_type)

def impute_df(df, col, impute_value=0):
    return df.loc[:, col].fillna(impute_value)
...
def preprocess_df(df, categorical_col_list, numerical_col_list, predictor, categorical_impute_value='nan',             numerical_impute_value=0):
    df.loc[:,predictor] = df.loc[:,predictor].astype('float32')
    for c in categorical_col_list:
        df.loc[:,c] = cast_df(df, c, d_type=str)
        df.loc[:,c] = impute_df(df, c, categorical_impute_value)
        df.loc[:, c]= df.loc[:, c].replace('?', 'nan')
    for numerical_column in numerical_col_list:
        df.loc[:,numerical_column] = cast_df(df, c, d_type=float)
